...
- **The Anvil** (`/app/project/:id/anvil`): **Quill** rich-text drafts (HTML in `project_sections.body`); plain-text drafts are migrated to paragraphs on load. Autosave; **split rail** — **score strip** (Weak / Moderate / Improving / Strong per category), **feedback & suggestions** (Bedrock when configured, Apply/Ignore), **citations** with **Insert citation**; **export** this section or whole project — [Anvil vision](docs/anvil-vision.md). **AWS Bedrock:** [docs/aws-bedrock.md](docs/aws-bedrock.md).
