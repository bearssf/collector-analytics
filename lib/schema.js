...
/** Idempotent — safe to call from request handlers if startup migration has not run yet. */
async function ensureResearchStage1Tables(getPool) {
  await queryRaw(
    getPool,
...
/** Stage 2 retrieval corpus for enrichment (Stage 2b). */
async function ensureResearchStage2Table(getPool) {
  await queryRaw(
    getPool,
    `CREATE TABLE IF NOT EXISTS research_stage2_corpus (
      id BIGINT AUTO_INCREMENT PRIMARY KEY,
      user_id INT NOT NULL,
      project_type VARCHAR(32) NOT NULL,
      corpus_json LONGTEXT NOT NULL,
      statistics_json LONGTEXT NOT NULL,
      created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
      CONSTRAINT fk_rs2c_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
      INDEX ix_rs2c_user_created (user_id, created_at DESC)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`
  );
}
...
module.exports = { ensureCoreSchema, ensureResearchStage1Tables, ensureResearchStage2Table };
