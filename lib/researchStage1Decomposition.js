...
function extractBalancedJsonObject(s) {
  ...
  if (start === -1) {
    throw new Error('No JSON object start {');
  }
  let depth = 0;
  let inString = false;
  let escape = false;
  ...
  throw new Error('Incomplete JSON object (unbalanced braces or truncated string)');
}
...
async function runStage1Decomposition(opts) {
  ...
  const maxCap = Math.min(
    parseInt(process.env.STAGE1_MAX_OUTPUT_TOKENS_CAP || '16384', 10) || 16384,
    64000
  );
  const defaultMax = parseInt(process.env.STAGE1_MAX_OUTPUT_TOKENS || '8192', 10) || 8192;
  const maxTokens = Math.min(
    maxCap,
    opts.maxTokens != null ? opts.maxTokens : defaultMax
  );

  const { text: raw, stopReason } = await invokeClaudeWithSystem(STAGE1_SYSTEM_PROMPT, userMessage, {
    maxTokens,
  ...
  try {
    ...
    let msg = `Could not parse JSON from model: ${e.message || e}`;
    if (stopReason === 'max_tokens') {
      msg +=
        ' The model hit the output token limit (response was truncated mid-JSON). Try project type "assignment" or "conference", fewer keywords, or set STAGE1_MAX_OUTPUT_TOKENS in the environment if your Bedrock model allows a higher max (see docs).';
    }
