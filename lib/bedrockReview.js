...
function parseBedrockInvokeBody(raw) {
  ...
  if (s.startsWith('<')) {
    throw new Error(
      'Bedrock returned HTML/XML instead of JSON — often a proxy error page or misrouted request. Check AWS region, model or inference profile id, and credentials.'
    );
  }
  try {
    return JSON.parse(s);
  } catch (e) {
    throw new Error(`Failed to parse Bedrock response as JSON: ${e.message || e}`);
  }
}
...
async function invokeClaudeMessages(prompt, opts = {}) {
  ...
  const parsed = parseBedrockInvokeBody(raw);
 ...
 * @param {{ maxTokens?: number, temperature?: number }} [opts]
 */
async function invokeClaudeWithSystem(systemPrompt, userMessage, opts = {}) {
  const region = trimBedrockEnv(process.env.AWS_REGION);
  const modelId = resolveBedrockModelId();
  ...
  const payload = {
    anthropic_version: 'bedrock-2023-05-31',
    max_tokens: opts.maxTokens != null ? opts.maxTokens : 8192,
    temperature: opts.temperature != null ? opts.temperature : 0.2,
    system: systemPrompt,
