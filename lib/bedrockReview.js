 ...
 *
 * Precedence: `BEDROCK_INFERENCE_PROFILE_ARN` (if non-empty) → else `BEDROCK_MODEL_ID`.
 * If the profile var is missing/empty, the old foundation model id may still be used and AWS can return "invalid model identifier".
 */
function resolveBedrockModelId() {
  const profile = trimBedrockEnv(process.env.BEDROCK_INFERENCE_PROFILE_ARN);
